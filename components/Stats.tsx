    ...
    <section className="py-16 md:py-24 bg-stone-50 border-t border-neutral-200 scroll-mt-28" id="stats">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex flex-col md:grid md:grid-cols-4 gap-8 md:gap-8 items-start">
            ...
            {/* Data Blocks - Optimized for Mobile (2 Columns) */}
            <div className="w-full md:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-8 md:gap-0">
                {content.items.map((item, index) => (
                    <div 
                        key={index} 
                        className={`
                            flex flex-col md:px-10
                            ${/* Desktop: Right border for all except last */ ''}
                            ${index !== content.items.length - 1 ? 'md:border-r border-neutral-300' : ''}
                            ${/* Mobile: Make the last item span full width if odd number */ ''}
                            ${(index === content.items.length - 1 && content.items.length % 2 !== 0) ? 'col-span-2 md:col-span-1' : 'col-span-1'}
                        `}
                    >
                        <div className="mb-4 md:mb-6 flex items-center justify-between">
                             <span className="text-[10px] md:text-xs font-bold text-neutral-400 uppercase tracking-widest font-sans">
                                {item.label}
                            </span>
                        </div>

                        <div className="mt-auto">
                            <h3 className="text-4xl md:text-5xl lg:text-6xl font-serif font-medium text-black mb-2 tracking-tight">
                                {item.value}
                            </h3>
                            <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-4">
                                ...
                                <p className="text-neutral-600 text-[10px] md:text-xs font-serif italic border-b border-neutral-200 pb-0.5">
                                    {item.subLabel}
                                </p>
                            </div>
                        </div>
